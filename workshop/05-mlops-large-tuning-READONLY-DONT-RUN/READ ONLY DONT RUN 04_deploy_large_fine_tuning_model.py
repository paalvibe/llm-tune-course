...
# MAGIC %sh mkdir -p /tmp/$EMAIL/$LARGE_TUNED_MODEL ; rm -r /tmp/$EMAIL/$LARGE_TUNED_MODEL/* ; cp $T5_LARGE_SUMMARY_MODEL_PATH/* /tmp/$EMAIL/$LARGE_TUNED_MODEL 2>/dev/null
...
envsetup.LARGE_TUNED_ML_EXPERIMENT_PATH

# COMMAND ----------

# Define an experiment path to log the model and register it
mlflow.set_experiment(envsetup.LARGE_TUNED_ML_EXPERIMENT_PATH)
last_run_id = mlflow.search_runs(filter_string="tags.mlflow.runName	= 't5-large-fine-tune-reviews'")['run_id'].item()
...
MODEL_NAME = envsetup.LARGE_TUNED_MODEL_UC
registered_model_name = MODEL_NAME
# "sean_t5_large_fine_tune_reviews"
...
artifacts_path = f"/tmp/{envsetup.EMAIL}/{envsetup.LARGE_TUNED_MODEL}/"
...
UC_PATH = envsetup.LARGE_TUNED_MODEL_UC
...
loaded_model = mlflow.pyfunc.load_model(f"models:/trainingmodels.dev_paldevibe_llmtopptur.t5-large-summary/8")
...
# MAGIC ### Deployment
# MAGIC
# MAGIC Once the model is registered, we can use API to create a Databricks GPU Model Serving Endpoint that serves the `t5-large-summary` model.

# COMMAND ----------

# Provide a name to the serving endpoint
endpoint_name = envsetup.LARGE_TUNED_MODEL
databricks_url = dbutils.notebook.entry_point.getDbutils().notebook().getContext().apiUrl().getOrElse(None)
token = dbutils.notebook.entry_point.getDbutils().notebook().getContext().apiToken().getOrElse(None)

# COMMAND ----------

####################### To save money, don't deploy the endpoint ######################
raise Exception("Don't deploy the endpoint, use the one already deployed!")
#######################################################################################
